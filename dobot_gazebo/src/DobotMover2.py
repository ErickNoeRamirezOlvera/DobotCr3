...
#Open Gripper
action = "Open"
action_pub.data = action
pub_action.publish(action_pub)
...
pose_goal.orientation.x = np.sin((np.pi/2)/2)
pose_goal.orientation.w = np.cos((np.pi/2)/2)
...
print("Point 2")
pose_goal = geometry_msgs.msg.Pose()
pose_goal.orientation.x = np.sin((np.pi)/2)
pose_goal.orientation.w = np.cos((np.pi)/2)
pose_goal.position.x = 0.0
